class AppElement{
	...
	static getElementID(variable, id) {
		if(this.vars === undefined){
			return undefined;
		}

		if(this.vars[variable] === undefined){
			this.vars[variable] = document.getElementById(id);
...
function init(this_) {
	if(!this_.constructor.hasOwnProperty('service')){
		this_.constructor.service = {};
	}

	if(!this_.constructor.hasOwnProperty('vars')){
		this_.constructor.vars = {};
	}
}
